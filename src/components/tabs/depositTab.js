...
function DepositTab() {
  ...
  useEffect(() => {
    ...
    else status = status && wallet.balances.HEGICBalance.value.lt(LOT_PRICE);
    setDepositButtonEnabled(status);
  }, [wallet.balances.HEGICBalance, allowanceIsZero, amountToDeposit, LOT_PRICE]);
  ...
  useEffect(() => {
    if (!!account && !!library) {
      ...
      stakingPool.totalBalance().then((balance) => setTotalBalance(balance));
      stakingPool.lockedBalance().then((balance) => setLockedBalance(balance));
      stakingPool.totalNumberOfStakingLots().then((n) => setTotalNumberOfStakingLots(n.toNumber()));
      stakingPool.numberOfStakingLots(Asset.ETH).then((n) => setNumberOfStakingLotsETH(n.toNumber()));
      stakingPool.numberOfStakingLots(Asset.WBTC).then((n) => setNumberOfStakingLotsWBTC(n.toNumber()));
    }
  }, [Asset.ETH, Asset.WBTC, account, library, stakingPool, wallet.balances]);

  const waitAndUpdate = async (txRequest, message) => {
    let pendingTxMessageTemplate = `https://rinkeby.etherscan.io/tx/${txRequest.hash}`;
    const { emitter } = displayNotification({ hash: txRequest.hash });

    emitter.on("txPool", ({ blockNumber }) => {
      return { message: pendingTxMessageTemplate, type: "pending" };
    });
  ...
  const allow = async () => {
    ...
    await waitAndUpdate(txRequest, "$HEGIC approved");
  };
  ...
  const depositHegic = async () => {
    let amount;
    if (amountToDeposit == "") amount = wallet.balances.HEGICBalance.value;
    else amount = ethers.utils.parseEther(amountToDeposit);

    const txRequest = await stakingPool.deposit(amount);
    setAmountToDeposit(0);
    await waitAndUpdate(txRequest, "$HEGIC deposited");
  };
    ...
    <Row>
      <Col sm="12">
        <Card body>
          ...
          <CardText>
            <Badge color="secondary">{totalNumberOfStakingLots} Staking Lots Purchased</Badge>{" "}
            <Badge color="primary">{numberOfStakingLotsETH} ETH Lots</Badge>{" "}
            <Badge color="warning">{numberOfStakingLotsWBTC} BTC Lots</Badge>
            ...
            Deposit your HEGIC in the pool and start earning fees generated by the Hegic Protocol.
            <br />
            &nbsp;&nbsp;1. Deposit your HEGIC
            <br />
            &nbsp;&nbsp;2. You start earning ETH and WBTC right away. <br />
            &nbsp;&nbsp;3. Yes, both ETH and WBTC. 🧙‍♂️ Wizardy? 🧙‍♂️
            <br />
          ...
          <InputGroup style={{ marginTop: "15px" }}>
            <Input
              placeholder={formatBN(wallet.balances.HEGICBalance.value)}
          ...
          <Button
            ...
            disabled={!depositButtonEnabled ? true : false}
            onClick={depositHegic}
          >
            <b>DEPOSIT</b>
          </Button>
          <StatusMsg />
          <div style={{ textAlign: "center", lineHeight: "80%" }}>
            ...
            <br />
            <span style={{ fontSize: "12px", color: "#667fcc" }}>
              <sup>*</sup>There is a 24h lock period after each Staking Lot purchase.
            </span>
          </div>
        </Card>
      </Col>
