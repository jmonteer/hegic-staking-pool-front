...
function YourSharesTab() {
  ...
  useEffect(() => {
    ...
    else status = status && wallet.balances.sHEGICBalance.value.lt(LOT_PRICE);
    setWithdrawButtonEnabled(status);
  }, [wallet.balances.sHEGICBalance, amountToWithdraw, LOT_PRICE]);
  ...
  const claimProfit = async () => {
    ...
    await waitAndUpdate(txRequest, "All profit claimed");
  };
  ...
  useEffect(() => {
    if (!!account && !!library) {
      stakingPool.profitOf(account, Asset.ETH).then((profit) => {
        setProfitETH(profit);
        if (profit.isZero()) setClaimButtonDisabled(true);
      });
  ...
  const withdrawHegic = async () => {
    let amount;
    if (amountToWithdraw == "") amount = wallet.balances.sHEGICBalance.value;
    else amount = ethers.utils.parseEther(amountToWithdraw);
    console.log(amount.toString());
    const txRequest = await stakingPool.withdraw(amount);
    setAmountToWithdraw(0);
    await waitAndUpdate(txRequest, "$HEGIC withdrawn");
  };
  ...
  const StatusMsg = () => {
    return (
      <>
        <br />
        {statusMsg ? <Alert color="primary">{statusMsg}</Alert> : null}
      </>
    );
  };
      ...
      <Row>
        <Col sm="12">
          <Card body>
            ...
            <h5>
              Your share{" "}
              <Badge color="primary" style={{ fontSize: "9pt" }}>
                You have {truncateEtherValue(formatBN(wallet.balances.sHEGICBalance.value), 2)} sHEGIC
              </Badge>
            </h5>

            <InputGroup>
              <Input
                placeholder={formatBN(wallet.balances.sHEGICBalance.value)}
            ...
            <Button
              ...
              disabled={!withdrawButtonEnabled ? true : false}
              onClick={withdrawHegic}
            >
              <b>WITHDRAW</b>
            </Button>
            <Button
